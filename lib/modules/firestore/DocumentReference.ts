...
export type ObserverOnError = (snapshot: any) => void;
export type ObserverOnNext = (snapshot: any) => void;
...
function isOnNext(obj): obj is ObserverOnNext {
  return obj && isFunction(obj);
}

function isOnError(obj): obj is ObserverOnError {
  return obj && !isFunction(obj);
}

function isObserver(obj): obj is Observer {
...
export default class DocumentReference {
  ...
  onSnapshot(
    ...
    if (isOnNext(optionsOrObserverOrOnNext)) {
      if (!isOnError(observerOrOnNextOrOnError)) {
        throw new Error(
    ...
    } else if (isObserver(optionsOrObserverOrOnNext)) {
